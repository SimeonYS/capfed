...
class CcapfedPipeline:

    # Database setup
    conn = sqlite3.connect('capfed.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `capfed`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM capfed WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO capfed (date, title, link, content)"
